...
variable "DATA_DISK_SIZE" {
        type            = string
        default         = "30"
        description = "The size of data disk, in GB. At least one data disk size should be provided."
}
...
locals {
    ...
    # All volumes
	storage = jsondecode(file("${path.module}/files/asesyb_vm_volume_layout.json"))
	...
	volume_sizes = local.volume_sizes_raw != [] ? flatten([ for k in local.volume_sizes_raw : replace(replace(k, "<data_disk_size>", var.DATA_DISK_SIZE), "<swap_disk_size>", tostring(local.swap_size)) ]) : []
	...
	display_crt_storage = local.volume_structure !=  null ? { for k, v in local.crt_structure : k => { for j, m in v : j => replace(replace(m, "<data_disk_size>", var.DATA_DISK_SIZE), "<swap_disk_size>", tostring(local.swap_size)) if j != "lvm" && j != "fs_type" && j != "mount_point" }} : null
}
